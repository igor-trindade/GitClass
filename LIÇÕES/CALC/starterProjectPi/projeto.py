...
def salvar_json(captura, arquivo="captura.json"):
    # Carrega JSON existente ou cria lista vazia
    ...
    # Adiciona nova captura
    lista.append(captura)

    # Salva de volta como array, mas cada captura em linha separada
    with open(arquivo, "w") as f:
        f.write("[\n")
        for i, item in enumerate(lista):
            # json.dumps sem espaços internos
            f.write(json.dumps(item, separators=(",", ":")))
            # Se não for o último, adiciona vírgula
            if i < len(lista) - 1:
                f.write(",\n")
            else:
                f.write("\n")
...
def iniciar_metricas():
    metricas()
    threading.Timer(3, iniciar_metricas).start()  # reagenda a função
iniciar_metricas()